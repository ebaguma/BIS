...
function canapprove($rec,$cn='request') {
//	dump( "SELECT * from v_bc_approvals where request=".$rec." AND (nextapprover_role='APPROVE' or nextapprover_role='RE-ASSIGN' or nextapprover_role is null ) AND (decision='APPROVE' or decision='RE-ASSIGN' or (decision='REJECT' and approver_id=".user()->id.") or (decision='CREATE' and approver_level=1)) AND (nextapprover_id ='".user()->id."'  or nextapprover_rolealias in ('".implode(myroles(),"','")."')) and nextapprover_done is null");
//	exit;
	...
	nextapprover_done is null")->queryAll();
		...
		//echo "SELECT * from v_bc_approvals where nextapprover_done is null and nextapprover_role='QUERY' && approver_role='QUERY' and ".$cn."=".$rec;
		//exit;
		if(count($c) ==1) {
		 ...
		 (decision='REJECT' and approver_id=".user()->id.") or
		 (decision='CREATE' and approver_level=1)) AND
			approver_id=".user()->id." order by id desc limit 1")->queryAll();
