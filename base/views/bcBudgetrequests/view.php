...
<table class="items">
	<tr><th>#</th><th>Item</th><th>Quantity</th><th>Unit Amount (UGX)</th><th>Total (UGX)</th><th>Committed Funds</th><th>Pending Approvals</th><th>Running Balance</th>
...
foreach($c as $ln) {
	...
	echo "<tr class=$cl>
		...
		if($commited_funding) echo "<td class=iright>".CHtml::link(' [Reverse]',array('bcBudgetrequests/reverse',
								 'section'=>$ln['section'],'accountcode'=>$ln['accountid'],'item'=>$ln['itemid'],'requestid'=>$_REQUEST['id']))."   </td>";
						...
						<table>
							<tr><td colspan=2><strong>Decision:</strong></td></tr>
								<tr><td><select name='decision' onChange="showemployee(this.value);">
									...
									<?php if($ln['nextapprover_role']=='QUERY' && $ln['nextapprover_id'] ==user()->id) { ?><option value='REPLY'>Reply</option><?php }   if(canapprove($_REQUEST['id'],'request',$ln['nextapprover_rolealias'])) {?>
...
?>
		...
		<div class="cd-timeline-block">
			...
			<div class="cd-timeline-content">
				...
				<?php 
				$d=['RE-ASSIGN'=>'Re-assigning to ','QUERY'=>'Querying '];
