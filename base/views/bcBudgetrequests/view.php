...
if (is_sat()) {
	$commited_funding = app()->db->createCommand("SELECT count(*) from bc_itembudgets where reasonid='" . $_REQUEST['id'] . "' and status='COMMITED'")->queryScalar();
	if ($commited_funding)
		$reversed = app()->db->createCommand("SELECT count(*) from bc_itembudgets where reasonid='" . $_REQUEST['id'] . "' and reason=7")->queryScalar();
...
<div id="bc-budgetrequests-grid" class="grid-view">
	<table class="items">
		<tr>
			...
			<th>Item</th>
			<th>Quantity</th>
			<th>Unit Amount (UGX)</th>
			<th>Total (UGX)</th>
			<th>Committed Funds</th>
			<th>Pending Approvals</th>
			<th>Running Balance</th>
			<?= ($commited_funding || $reversed) ? "<th>Options</th>" : "" ?>
		</tr>
		<?php
		...
		foreach ($c as $ln) {
			...
			echo "<tr class=$cl>
		<td>" . $i . "</td>
		<td>" . $ln['itemname'] . CHtml::link(' [Details]', array(
				'bcItembudgets/accountcodes',
				'section' => $ln['section'], 'accountcode' => $ln['accountid'], 'item' => $ln['itemid']
		...
		<td class=iright>" . number_format($running[0]['a']) . "</td>";
			if ($commited_funding) echo "<td class=iright>" . CHtml::link(' [Reverse]', array(
				'bcBudgetrequests/reverse',
				'section' => $ln['section'], 'accountcode' => $ln['accountid'], 'item' => $ln['itemid'], 'requestid' => $_REQUEST['id']
			)) . "   </td>";
...
<section id="cd-timeline" class="cd-container">
	...
	?>
		<div class="cd-timeline-block">
			...
			<div class="cd-timeline-content" style="margin-right:75px">
				<h2><?= user()->details['names'] . " - " . $ln['nextapprover_rolealias'] ?></h2>
				<hr />
				<p style='text-align:justify'>
					...
					<form method=post enctype="multipart/form-data" id='comment-appove-form'>
						...
						<table>
							<tr>
								<td colspan=2><strong>Decision:</strong></td>
							</tr>
							<tr>
								<td><select name='decision' onChange="showemployee(this.value);">
										<option>- select -</option>
										<?php if ($ln['nextapprover_role'] == 'QUERY' && $ln['nextapprover_id'] == user()->id) { ?>
											<option value='REPLY'>Reply</option><?php }
																			if (canapprove($_REQUEST['id'], 'request', $ln['nextapprover_rolealias'])) { ?>
										...
										<!-- Edwin:8/05/202 Removed to restrict user to only reply
											<option value='QUERY'>Query</option> 
										-->
									</select></td>
								<td>
									<div id=toemp style='display:none; '>
										TO:
										<select name=emp style="width:150px">
											<option></option>
											<?php
							...
							<tr>
								<td colspan=2><textarea name=comments rows=10 cols=50 style="width:275px; resize:vertical"></textarea></td>
							</tr>
							<tr>
	...
	foreach ($c as $ln) {
		...
		/*** Edwin: 22/04/2020
		 * Yii::app()->user->budget['id'] Line 231, 237
		 ***/

		$dir = Yii::getPathOfAlias('webroot') . '/appendix/bc' . $ln['budget']; //.'/'.Yii::app()->user->id.'-'.$model->id.'-'.$m2->id.'-'.$pic->name)
		if (is_dir($dir)) {
			$s = scandir($dir);
			$p = '/^' . $ln['approver_id'] . '-' . $ln['request'] . '-' . $ln['id'] . '-/';
			foreach ($s as $d) {
				if (preg_match($p, $d))
					$att .= "<a href='appendix/bc" . $ln['budget'] . "/" . $d . "' target=_blank>" . $d . "</a><br/>";
			}
		}
		?>
		...
		<div class="cd-timeline-block">
			...
			<div class="cd-timeline-content">
				<h2><?= $ln['approvernames'] . " - " . $ln['approver_rolealias'] ?></h2>
				<?php
				//EDWIN:8/05/2020 Updating Description for Queries
				$d = ['RE-ASSIGN' => 'Re-assigning to ', 'QUERY' => 'Querying/Replying '];
				//echo $ln['nextapprovernames'];
				if (isset($ln['nextapprovernames']))
					echo "<em style='font-size:14px'>" . $d[$ln['decision']] . $ln['nextapprovernames'] . "</em>";
				?>
				<hr />
