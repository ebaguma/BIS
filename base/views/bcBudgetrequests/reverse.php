...
// test if the thing has been committed
if(is_sat()) {
	$commited_funding=app()->db->createCommand("SELECT count(*) from bc_itembudgets where reasonid='".$_REQUEST['requestid']."' and status='COMMITED'")->queryScalar();
	if($commited_funding)
		$reversed=app()->db->createCommand("SELECT count(*) from bc_itembudgets where reasonid='".$_REQUEST['requestid']."' and reason=7")->queryScalar();
...
<table class="items">
	<tr><th>#</th><th>Item</th><th>Quantity</th><th>Unit Amount (UGX)</th><th>Total (UGX)</th>
<?= ($commited_funding && !$reversed) ? "<th>Reverse</th>" : "" ?>
		<th>Committed Funds</th><th>Pending Approvals</th><th>Running Balance</th>

	</tr>
<?php

$c=app()->db->createCommand("SELECT * from v_bc_budgetrequests_items where requestid='".$_REQUEST['requestid']."'")->queryAll();
foreach($c as $ln) {
	...
	echo "<tr class=$cl>
		...
		if($commited_funding && !$reversed) echo "<td class=iright><form method=post><input name='reverse' value='".$lntotal."' /><input type='submit' value='Reverse' />
<input type=hidden name='r' value='bcBudgetrequests/reverse' />
...
<input type=hidden name='section' value='".$_REQUEST['section']."' />
		...
		echo "<td class=iright>".number_format($commited[0]['a']*-1)."</td><td class=iright>".number_format($pending[0]['a']*-1)."</td>
		<td class=iright>".number_format($running[0]['a'])."</td>";
	/*if($commited_funding) echo "<td class=iright>".CHtml::link(' [Reverse]',array('bcItembudgets/reverse',
							 'section'=>$ln['section'],'accountcode'=>$ln['accountid'],'item'=>$ln['itemid'],'requestid'=>$_REQUEST['requestid']), array('target'=>'_blank'))."   </td>";
	if($reversed) echo "<td class=iright>Reversed</td>";*/
	echo "</tr>";
}
$i++;
$cl=$i%2==0? "even" : "odd";
...
<?php
	$finalised=1;
	$c1=app()->db->createCommand("SELECT * from v_bc_approvals where request='".$_REQUEST['requestid']."' order by approverdate desc limit 1")->queryAll();
...
$c=app()->db->createCommand("SELECT * from v_bc_approvals where request='".$_REQUEST['requestid']."' and nextapprover_done is null")->queryAll();
$ln=$c[0];
if(cancomment($_REQUEST['requestid'])) {
?>
			...
			<div class="cd-timeline-content">
				...
				<p style='text-align:justify'>
					...
					<form method=post enctype="multipart/form-data" id='comment-appove-form'>
						<input type='hidden' name='r' value='bcBudgetrequests/view&id=<?=$_REQUEST['requestid']?>'>
						<input type='hidden' name='id' value='<?=$_REQUEST['requestid']?>'>
						<table>
							<tr><td colspan=2><strong>Decision:</strong></td></tr>
								<tr><td><select name='decision' onChange="showemployee(this.value);">
									<option>- select -</option>
									<?php if($ln['nextapprover_role']=='QUERY' && $ln['nextapprover_id'] ==user()->id) { ?><option value='REPLY'>Reply</option><?php }   if(canapprove($_REQUEST['requestid'])) {?>
...
$c=app()->db->createCommand("SELECT * from v_bc_approvals where request='".$_REQUEST['requestid']."' order by approverdate desc")->queryAll();
foreach($c as $ln) {
	$att="";
	$dir=Yii::getPathOfAlias('webroot').'/appendix/bc'.Yii::app()->user->budget['id'];
	if(is_dir($dir)) {
		$s=scandir($dir);
