...
switch ($_REQUEST['pc']) {
	...
	break;
	case 	"42":
	$sql ="accountcode regexp '^".$_REQUEST['pc']."$' or accountcode regexp '^43$'";
	break;
	default:
	$sql ="accountcode regexp '^".$_REQUEST['pc']."$'";
}
$rd='realdept';
...
$yrs=array(3=>2016,4=>2017,5=>2018,6=>2019);
$ddpt=!empty($_REQUEST['ddpt'])? $_REQUEST['ddpt'] : user()->dept[department];
?>
<style>
.above {
	color:red;
...
<h1>Multi-Year <?php echo $bname;?> Budget</h1>
<form>
Select Department: <select name='ddpt'>

<option value=''>- Select - </option>
<?php
	$depts=app()->db->CreateCommand("SELECT * from dept")->queryAll();
	foreach ($depts as $d) {
		$s=$_REQUEST['ddpt']==$d['id'] ? "SELECTED=SELECTED" : "";
		echo "<option value='".$d['id']."' $s>".$d['dept']."</option>";
	}
?>	
</select>
<input type='submit' value='Select' />
<input type='hidden' value='1' name=old />
<input type='hidden' value='<?=$_REQUEST[pc]?>' name=pc />
<input type='hidden' value='<?=$_REQUEST[p]?>' name=p />
<input type='hidden' value='<?=$_REQUEST[r]?>' name=r />
</form>
<?php 
...
foreach($yrs as $yr=>$yrn) 
	$aht.="<th style='width:100px'>".$yrn."</th>";
$aht.="<th>Total</th></tr>";
foreach($cs as $c) {
...
foreach($codes as $cd) {
	...
	$dctr=0;
	foreach($yrs as $yr=>$yrn) {
		$bdgt=Yii::app()->db->createCommand("select sum(amount) a from v_budget where accountcode ='".$cd[id]."' and budget='".$yr."' and realdept=".$ddpt)->queryAll();
		$total += $bdgt[0][a];
		$linetot +=$bdgt[0][a];
		$taccd += $bdgt[0][a];
...
if($_REQUEST['c']) {
	switch ($_REQUEST['c']) {
		case "400004":// NSSF
			nssf_calc(); // do nssf calcs
			report_400001();
			break;
		case "400008":// Gratuity
			gratuity_calc(); // do nssf calcs
			report_400001();
			break;
...
function r_repairs() {
	...
	foreach($qdep as $c) {
		...
		$ht.="<tr class=$cl>
					...
					<td class=ri>".number_format($c['fuelprice'])."</td>
					<td class=ri>".number_format($c['fueltotalprice'])."</td>
					<td class=ri>".number_format($c['fueltotalprice']/10)."</td>
				</tr>";
	}
	$cl=$ct %2==0? "odd" : "even"; $ct++;
	$ht.="<tr class=$cl>
				...
				<td>&nbsp;</td><td>&nbsp;</td>
				<td class=ri>&nbsp;</td>
				<td class=ri><strong>".number_format($tt/10)."</strong></td>
			</tr>";
...
function standby_calc() {
	...
	foreach($emp as $em) {
		...
		// /echo $emp_bt[0][a]."<br>";
		$af=Items::model()->findByAttributes(array('accountcode'=>78,'name'=>$em->checkno." - ".$em->employee." - ".$em->designation));
		if($af==null) {
			$af=new Items;
			$af->attributes=array('accountcode'=>78,'name'=>$em->checkno." - ".$em->employee." - ".$em->designation);
			$af->save();
		}
...
function shift_calc() {
	...
	foreach($emp as $em) {
		$emp_bt=app()->db->createCommand("SELECT price a from v_budget where tbl='employees' and tblid='".$em->id."' and budget='".budget()."' and tblcolumn='salary'")->queryAll();
		// /echo $emp_bt[0][a]."<br>";
		$af=Items::model()->findByAttributes(array('accountcode'=>90,'name'=>$em->checkno." - ".$em->employee." - ".$em->designation));
		if($af==null) {
			$af=new Items;
			$af->attributes=array('accountcode'=>90,'name'=>$em->checkno." - ".$em->employee." - ".$em->designation);
			$af->save();
		}
		$ap=ItemsPrices::model()->findByAttributes(array('item'=>$af->id,'budget'=>budget()));
		if($ap==null)
...
function leave_calc() {
	...
	//echo $emp_bt[0][a]." ".$em->employee."<br>";
		$af=Items::model()->findByAttributes(array('accountcode'=>81,'name'=>$em->checkno." - ".$em->employee." - ".$em->designation));
		if($af==null) {
			$af=new Items;
			$af->attributes=array('accountcode'=>81,'name'=>$em->checkno." - ".$em->employee." - ".$em->designation);
			$af->save();
		}
		$ap=ItemsPrices::model()->findByAttributes(array('item'=>$af->id,'budget'=>budget()));
		if($ap==null)
...
function gratuity_calc() {
	...
	foreach($emp as $em) {
		...
		// /echo $emp_bt[0][a]."<br>";
		$af=Items::model()->findByAttributes(array('accountcode'=>79,'name'=>$em->checkno." - ".$em->employee." - ".$em->designation));
		if($af==null) {
			$af=new Items;
			$af->attributes=array('accountcode'=>79,'name'=>$em->checkno." - ".$em->employee." - ".$em->designation);
			$af->save();
		}
		$ap=ItemsPrices::model()->findByAttributes(array('item'=>$af->id,'budget'=>budget()));
		if($ap==null)
...
function nssf_calc() {
	...
	foreach($emp as $em) {
		...
		// /echo $emp_bt[0][a]."<br>";
		$af=Items::model()->findByAttributes(array('accountcode'=>75,'name'=>$em->checkno." - ".$em->employee." - ".$em->designation));
		if($af==null) {
			$af=new Items;
			$af->attributes=array('accountcode'=>75,'name'=>$em->checkno." - ".$em->employee." - ".$em->designation);
			$af->save();
		}
		$ap=ItemsPrices::model()->findByAttributes(array('item'=>$af->id,'budget'=>budget()));
		if($ap==null)
