...
foreach($codes as $cd) {
	$bdgt=Yii::app()->db->createCommand("select sum(amount*qty) a from budget where accountcode ='".$cd[id]."'")->queryAll();
	//if($bdgt[0][a] > 0) { 
		$total += $bdgt[0][a];
		$taccd += $bdgt[0][a];
		$ht.= "<tr><td><a class='reportlinks' href='?r=site/reports&p=adminexpenses&c=".$cd[accountcode]."'>".$cd[accountcode]."</a></td><td><a class='reportlinks'  href='?r=site/reports&p=adminexpenses&c=".$cd[accountcode]."'>".$cd[item]."</a></td><td>".number_format($bdgt[0][a])."</td></tr>";
		//}
}

$ht.="<tr><td colspan=2><h3>Total</h3></td><td><h3>".number_format($taccd)."/=</h3></td></tr>";
