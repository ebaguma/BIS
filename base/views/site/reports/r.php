	...
	foreach ($cs as $c) {
		...
		foreach ($codes as $cd) {
			...
			foreach ($depts as $dep) {
				...
				$tic = $dc == "above" ? "title='" . number_format($cap[$dep['id']][$cd[accountcode]]) . "'" : "";

				$ht .= "<td style='text-align:right' class=$dc $tic>" . number_format($bdgt[0][a]) . "</td>";
...
function report_allowances()
{
	...
	foreach ($qdep as $dep) {
		...
		foreach ($cs as $csd) {
			...
			if ($csd['department'] == $dep[id]) {
				...
				if ($csd['sectionname'] != $s) {
					$s = $csd['sectionname'];
					if ($sectotal > 0) echo "<tr><td colspan=5><strong>Section Total</strong></td><td style='text-align:right'></td><td style='text-align:right'><strong>" . number_format($sectotal) . "</strong></td></tr>";
		...
		echo "<tr><td colspan=5><strong>Department Total</strong></td><td style='text-align:right'></td><td style='text-align:right'><strong>" . number_format($deptotal) . "</strong></td></tr>";
	}
	echo "<tr><td colspan=5><strong>Grand Total</strong></td><td style='text-align:right'></td><td style='text-align:right'><strong>" . number_format($tot) . "</strong></td></tr>";

	if ($_REQUEST['print'] != 1)  echo "<tr><td colspan=5><a href='" . $_SERVER['REQUEST_URI'] . "&print=1'>View in Report Format</a></td><td style='text-align:right'><strong>&nbsp;</strong></td></tr>";
...
function report_400001()
{
	...
	if (is_dept_head() && !corporate_report())
		$ad = " and department=(select department from sections where id='" . user()->dept[id] . "')";
	if (!is_dept_head() && !corporate_report() && !is_hr())
		$ad = " and section='" . user()->dept[id] . "'";
	// EDWIN (31/03/2021): Human Resource	
	if (is_hr() && !corporate_report())
		$ad = "";
	...
	foreach ($qdep as $dep) {
		$ct2 = 1;
		echo "<tr><td colspan=8><h2>Department: " . $dep['dept'] . "</h2></td></tr>";
		echo $thead;
		$s = "";
		...
		foreach ($cs as $csd) {
			if ($csd['department'] == $dep[id]) {
				...
				if ($csd['sectionname'] != $s) {
					$s = $csd['sectionname'];
					if ($sectotal > 0) echo "<tr><td colspan=7><strong>Section Total</strong></td>
						" . (!in_array($_REQUEST['c'], $annual) ? "<td style='text-align:right'><strong>" . number_format($sectotal) . "</strong></td>" : "") . "
						<td style='text-align:right'><strong>" . number_format($tsectotal) . "</strong></td></tr>";
					echo "<tr><td colspan=7><h3>Section: " . $csd['sectionname'] . "</h3></td></tr>";
		...
		if ($sectotal > 0) echo "<tr><td colspan=7><strong>Section Total</strong></td>
			" . (!in_array($_REQUEST['c'], $annual) ? "<td style='text-align:right;font-weight:bold'><strong>" . number_format($sectotal) . "</strong></td>" : "") . "
			<td style='text-align:right'><strong>" . number_format($tsectotal) . "</strong></td></tr>";

		echo "<tr><td colspan=7><strong>Department Total</strong></td>
			" . (!in_array($_REQUEST['c'], $annual) ? "<td style='text-align:right;font-weight:bold'><strong>" . number_format($deptotal) . "</strong></td>" : "") . "
		<td style='text-align:right;font-weight:bold'><strong>" . number_format($tdeptotal) . "</strong></td></tr>";
	}
	echo "<tr><td colspan=7><strong>Grand Total</strong></td>
		" . (!in_array($_REQUEST['c'], $annual) ? "<td style='text-align:right;font-weight:bold'><strong>" . number_format($tot) . "</strong></td>" : "") . "
	<td style='text-align:right;font-weight:bold'><strong>" . number_format($ttot) . "</strong></td></tr>";

	if ($_REQUEST['print'] != 1)  echo "<tr><td colspan=7><a href='" . $_SERVER['REQUEST_URI'] . "&print=1'>View in Report Format</a></td><td style='text-align:right'><strong>&nbsp;</strong></td></tr>";
	echo "</tbody></table></div>";
}
function standby_calc()
{
	...
	foreach ($emp as $em) {
		...
		if ($em->standby == 1) {
			...
			$bt->attributes = array('budget' => budget(), 'item' => $af->id, 'dept' => $em->section, 'qty' => 1, 'tbl' => 'employees', 'tblcolumn' => 'standby', 'tblid' => $em->id, 'createdby' => user()->id, 'createdon' => date("Y-m-d"), 'dateneeded' => date("Y-m-d"), 'period' => budget('period'));
			$bt->save();
		} else
...
function shift_calc()
{
	...
	foreach ($emp as $em) {
		...
		if ($em->shift == 1) {
			if ($bt == null) $bt = new Budget;
			$bt->attributes = array('budget' => budget(), 'item' => $af->id, 'dept' => $em->section, 'qty' => 1, 'tbl' => 'employees', 'tblcolumn' => 'shift', 'tblid' => $em->id, 'createdby' => user()->id, 'createdon' => date("Y-m-d"), 'dateneeded' => date("Y-m-d"), 'period' => budget('period'));
			$bt->save();
		} else
			if ($bt != null) $bt->delete();
	}
...
function leave_calc()
{
	...
	foreach ($emp as $em) {
		...
		if ($ap == null)
			$ap = new ItemsPrices;
		$ap->attributes = array('currency' => '1', 'item' => $af->id, 'price' => intval($emp_bt[0][a] * budget('period') / 12));
		//dump($ap->attributes,false);
		if (!$ap->save()) dump($ap->errors);
		$bt = Budget::model()->findByAttributes(array('budget' => budget(), 'item' => $af->id));
		if ($em->contract == 1) {
...
function rgeneral()
{
	...
	$ht .= "<tr><td><strong>Total</strong></td><td colspan=3><strong>" . number_format($tot) . "</strong></td></tr>";
	$ht .= "</table></div>";
...
function report_440006()
{
	...
	foreach ($mysecs as $mysec) {
		...
		foreach ($cs as $csd) {
			...
			$dhsub .=  "<tr><td style='border:0px solid blue;' colspan='4'><strong>Operational Materials</strong></td></tr>";
			$dhsub .=  "<tr><td><strong>Item</strong></td><td></td><td><strong>Quantity</strong></td><td><strong>Unit Price</strong></td><td><strong>Total</strong></td></tr>";
			$det = Yii::app()->db->createCommand("select * from v_subsistence_details where activity='" . $csd['id'] . "'")->queryAll();
			$mtot = $subt = 0;
			foreach ($det as $e) {
				$tot = $e['quantity'] * $e['price'];
				$mtot += $tot;
				$cl2 = $ct2 % 2 == 0 ? "odd2" : "even2";
				$ct2++;
			...
			$dhsub .= "<tr><td colspan=2><strong>Total</strong></td><td>&nbsp;</td><td>&nbsp;</td><td style='text-align:right'><strong>" . number_format($mtot) . "</strong></td></tr>";
			$dhsub .= "<tr><td colspan='4'><strong>Subsistence</strong></td></tr>";
			$dhsub .= "<tr><td><strong>Staff</strong></td><td><strong>Salary Scale</strong></td><td><strong>Rate</strong></td><td><strong>Days</strong></td><td><strong>Total</strong></td></tr>";
