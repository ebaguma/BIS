...
<div class='container'>
	...
	<?php } ?>
		...
		<div class="row">
			<div class='left'><?=CHtml::label('Cost Center: ',false)?><small><em> (Leave blank for entire budget)</em></small></div>	<div class='middle'>
			<?php
				...
				echo Chtml::dropDownList('costcentre', $_REQUEST['costcentre'], $role_list,
					array(
						'style'=>'width:270px',
	...
	?>
		...
		<div class="row">
			<div class='left'><?=CHtml::label('Account Code: ',false)?><small><em> (Leave blank to stop at cost centre level)</em></small></div>	<div class='middle'>
			<?php
		...
		<div class="row">
			<div class='left'><?=CHtml::label('Item: ',false)?> <small><em> (Leave blank to stop at account code level)</em></small></div>	<div class='middle'>
			<?php
...
<center><div id="bc-itembudgets-grid" class="grid-view">
...
<table style='width:70%'>
	<tr>
		<td><h4>Section:</h4></td>
		<td><h3><b><?=$model[0]['dept']?> - <?=$model[0]['sectionname']?></b></h3></td>
	</tr>
	<tr>
		<td><h4>Account Code:</h4></div>
		<td><h3><?=$model[0]['accountcode']?> - <?=$model[0]['accountitem']?></h3></td>
	</tr>
	<tr>
		<td><h4>Item:</h4></td>
		<td><h3><?=$model[0]['itemname']?></h3></td>
	</tr>
</table>
...
if($model && $_REQUEST['accountcode'] && !$_REQUEST['item']) {
	...
	<div id='foo'  class='grid-view'><table class="items">
	<thead>
	<tr>
		<th id="bc-itembudgets-grid_c0">Section</th>
		<th id="bc-itembudgets-grid_c1">Budget</th>
		...
		<?php
			foreach($rawData as $r) {
				...
				foreach($rawData2 as $r2) {
					$ctr++;
					$budget=Yii::app()->db->createCommand("Select sum(amount) from v_bc_itembudgets where budget=".budget()." and itemname='".$r2[itemname]."' and section='".$r['section']."' and accountid='".$_REQUEST['accountcode']."' and reason in (1)")->queryScalar();
					$realloc=Yii::app()->db->createCommand("Select sum(amount) from v_bc_itembudgets where budget=".budget()." and itemname='".$r2[itemname]."' and section='".$r['section']."' and accountid='".$_REQUEST['accountcode']."' and reason in (2,5,4)")->queryScalar();
					$commit=Yii::app()->db->createCommand("Select sum(amount) from v_bc_itembudgets where budget=".budget()." and itemname='".$r2[itemname]."' and section='".$r['section']."' and accountid='".$_REQUEST['accountcode']."' and `status`='COMMITED' and reason in (3,6)")->queryScalar();
					$pending=Yii::app()->db->createCommand("Select sum(amount) from v_bc_itembudgets where budget=".budget()." and itemname='".$r2[itemname]."' and section='".$r['section']."' and accountid='".$_REQUEST['accountcode']."' and `status`='PENDING' and  reason in (3,6)")->queryScalar();
					$balance=Yii::app()->db->createCommand("Select sum(amount) from v_bc_itembudgets where budget=".budget()." and  itemname='".$r2[itemname]."' and section='".$r['section']."' and accountid='".$_REQUEST['accountcode']."' ")->queryScalar();
					$budgets	+=$budget;
					$reallocs +=$realloc;
					$commits 	+=$commit;
					$pendings	+=$pending;
...
<?php
				...
				$sql2="select distinct accountcode,accountitem,accountid from v_bc_itembudgets where section='".$r['section']."' and budget='".budget()."'  and accountcode like '".$_REQUEST['accountcode']."%' order by accountcode";
				$rawData2 = Yii::app()->db->createCommand($sql2)->queryAll();
				$ctr=$commits=$reallocs=$budgets=$pendings=$balances=0;
				foreach($rawData2 as $r2) {
					$ctr++;
					$budget=Yii::app()->db->createCommand("Select sum(amount) from v_bc_itembudgets where budget=".budget()." and accountcode like '".$r2[accountcode]."%' and section='".$r['section']."'  and reason in (1)")->queryScalar();
					$realloc=Yii::app()->db->createCommand("Select sum(amount) from v_bc_itembudgets where budget=".budget()." and accountcode like '".$r2[accountcode]."%' and section='".$r['section']."'  and reason in (2,5,4)")->queryScalar();
					$commit=Yii::app()->db->createCommand("Select sum(amount) from v_bc_itembudgets where budget=".budget()." and itemname='".$r2[itemname]."' and section='".$r['section']."' and  `status`='COMMITED' and reason in (3,6)")->queryScalar();
					$pending=Yii::app()->db->createCommand("Select sum(amount) from v_bc_itembudgets where budget=".budget()." and itemname='".$r2[itemname]."' and section='".$r['section']."' and  `status`='PENDING' and  reason in (3,6)")->queryScalar();
					$balance=Yii::app()->db->createCommand("Select sum(amount) from v_bc_itembudgets where budget=".budget()." and  itemname='".$r2[itemname]."' and section='".$r['section']."'")->queryScalar();
					$budgets	+=$budget;
					$reallocs +=$realloc;
					$commits 	+=$commit;
					$pendings	+=$pending;
