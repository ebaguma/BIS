...
?>
	...
	if ($model && $_REQUEST['accountcode'] && !$_REQUEST['item']) {
		if (is_sat() || is_pbfo() || is_sys_admin()) {
			...
			$sql = "select distinct section,sectionname,dept,accountitem from v_bc_itembudgets where section='" . section() . "' and budget='" . budget() . "'  and accountid ='" . $_REQUEST['accountcode'] . "' order by dept,sectionname";
		}
		$rawData = Yii::app()->db->createCommand($sql)->queryAll();
	?>
