...
//		print_r(Yii::app()->user->budget['id']); exit;
//echo Yii::app()->user->dept['id']; exit;
	$redir=array(
	...
	//	$this->redirect('http://localhost/bis/?r='.$redir[$_REQUEST['StaffCosts']['accountcode']]);
		
	}
 	//echo $_REQUEST['StaffCosts']['accountcode'];
		$csz=""; $jarray="";$jbudget="";
		...
		function test(dd,v) { 
			...
			//var totalprice = document.getElementById("qty"+dd).value; //accounting.formatNumber(ParseInt(document.getElementById("qty"+dd).value))// *accounting.unformat(document.getElementById("price"+dd).innerHTML));
...
<a id="additional-link" href="#">Add Item</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a id="additional-linkr" href="#" style="color:red">Remove Last Item</a><table class=items><tr><th>No.</th><th>Item</th><th>Price</th><th>Quantity</th><th>Total</th></tr><tbody id="additional-inputs">
	...
	<tr class="+rown+" id=sel<?php echo $d;?>><td><?php echo $d;?></td><td><select onchange='test(<?php echo $d; ?>,this.value);' name=StaffCosts[item][]><?php echo $csz?></select></td><td id=price<?php echo $d;?>></td><td><input id=qty<?php echo $d;?> onBlur='test2(<?php echo $d;?>)' size=4 type=number value=1 name=StaffCosts[quantity][]></td><td id=total<?php echo $d;?>></td></tr>
