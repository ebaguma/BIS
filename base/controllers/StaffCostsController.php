...
class StaffCostsController extends Controller
{
	...
	<script type="text/javascript">
		...
		jQuery(function ($) {
			...
			$("#additional-link").bind("click", function () { //document.getElementById('price"+d+"').innerHTML=this.class
				...
				$("#additional-inputs").append("\n<tr class=" + rown + " id=sel" + d + "><td>" + d + ".</td><td><select id='it" + d + "' style='width:350px' onchange='test(" + d + ",this.value);' name=StaffCosts[item][]><option value=0> - select -</option><?php echo $csz?></select></td><td><input id='dateneeded" + d + "' name=StaffCosts[dateneeded][] placeholder='YYYY-MM-DD' /></td><td style='text-align:right' id=price" + d + "></td><td id=cur" + d + "></td><td><select onChange='test2(" + d + ")'  id='period" + d + "' name=StaffCosts[period][]><option value=Annually>Annually</option><option value=Monthly>Monthly</option></select></td><td><input id=qty" + d + " onKeyUp='test2(" + d + ");' onChange='test2(" + d + ")'  style='width:60px' type=number value=1 name=StaffCosts[quantity][]></td><td style='text-align:right' id=total" + d + "></td></tr>");
	...
	$dat = Yii::app()->db->createCommand()
			->select('*')
			->from('v_staff_costs')
			->where('accountcode like :code', array(':code'=>$_REQUEST['StaffCosts']['accountcode']))
			->andWhere('budget like :budget', array(':budget'=>Yii::app()->user->budget['id']))
			->andWhere('dept like :dept', array(':dept'=>user()->dept['id']))
			->queryAll();
				...
				<tr class=<?php echo $cl?> id=sel
					<?php echo $d;?>>
						...
						<?php 
							...
							?> 
							<input value=<?php echo $dt[dateneeded] ?> name=StaffCosts[dateneeded][] placeholder="YYYY-MM-DD" />
						</td>
