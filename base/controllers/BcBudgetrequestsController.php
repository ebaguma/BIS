...
//		echo "NO!";exit;
		...
		if(!empty($comments) && in_array($decision,$validdecisions) && $id) {
			...
			if(!$m->save())  { echo "Failed to Save! Please report this error to the administrator"; dump($m->getErrors()); }
		...
		$mail->setFrom(user()->details['email'], user()->details['names']);
				...
				case "APPROVE":
					...
					if($mm[0]) {
						...
						foreach($nextapprover_actual as $a) {
							$okay = preg_match(
							        '/^[A-z0-9_\-\.]+[@][A-z0-9_\-]+([.][A-z0-9_\-]+)+[A-z.]{2,4}$/', $a['email']
							);
							if((
								(
			...
			if(!$m2->save()) { echo "failed to save! the next approver will NOT be able to approve. Please send this error to the administrator"; dump($m2->getErrors()); }
